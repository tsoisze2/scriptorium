...
const MyReplyReports: React.FC = () => {
    const [reports, setReports] = useState<ReportReply[]>([]);
    ...
    const fetchReports = async (page: number = 1) => {
        ...
        try {
            ...
            const response = await axios.post<PaginatedResponse>(
                "/api/report/reply/searchMyReports",
                { page },
                {
                    headers: {
    ...
    const handleDelete = async (reportId: number) => {
        try {
            ...
            const response = await axios.delete("/api/report/reply/delete", {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                data: { reportReplyId: reportId }, // Pass the reportReplyId in the request body
            });
    ...
    return (
        <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-4">My Reply Reports</h2>
                            ...
                            {reports.map((report) => (
                                ...
                                >
                                    <div>
                                        ...
                                        <strong>Reply ID:</strong> {report.replyId}
                                    </div>
                                    <button
