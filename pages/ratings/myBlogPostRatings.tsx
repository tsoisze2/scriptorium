...
const MyBlogPostRatings: React.FC = () => {
  ...
  const fetchRatings = async (page: number = 1) => {
    ...
    try {
      ...
      const response = await axios.post<PaginatedResponse>(
        "/api/rating/blogPost/searchMyRatings",
        { page },
        {
          headers: {
  ...
  const handleDelete = async (ratingId: number) => {
    try {
      ...
      const response = await axios.delete("/api/rating/blogPost/delete", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        data: { ratingBlogPostId: ratingId }, // Pass ratingBlogPostId in request body
      });
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-4">My Blog Post Ratings</h2>
              ...
              {ratings.map((rating) => (
                ...
                >
                  <div>
                    ...
                    <strong>Blog Post ID:</strong> {rating.blogPostId}
                  </div>
                  <button
...
export default MyBlogPostRatings;
